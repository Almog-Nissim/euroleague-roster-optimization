    ...
    <div dir="rtl" className="hl">
      ...
      <header className="top">
        <div className="eyebrow">
          יורוליג · {d.benchmark.n} קבוצות · {"עונות "}{d.benchmark.seasons.map(sname).join(" ו-")}
        </div>
        <h1 className="ask">
          כמה ניצחונות אפשר להוסיף בלי להוציא יותר כסף?
        </h1>
        <div className="answer">
          <span className="huge blue" dir="ltr">+{f(H.value, 1)}</span>
          <span className="unit">ניצחונות בעונה, בממוצע</span>
        </div>
        <p className="lede">
          נתנו למנוע בדיוק את התקציב של כל קבוצה ביורוליג, ובדקנו מה היה
          קורה אם הוא היה בוחר את השחקנים. הטווח הסביר הוא בין{" "}
          <b>{f(H.ci[0], 1)}</b> ל-<b>{f(H.ci[1], 1)}</b> ניצחונות. הסגל של
          המנוע יצא טוב יותר מהסגל האמיתי ב-<b>{d.benchmark.n_win} מתוך{" "}
          {d.benchmark.n}</b> הבדיקות.
        </p>
      </header>
      ...
      {club && (
        <section className="card duel">
          ...
          <p className="sub">
            ל{cname(club)} היה בעונת {sname(club.season)} תקציב של{" "}
            <b>{bpct(club.budget, AVGS[club.season])}% מקבוצה ממוצעת</b>. המנוע
            קיבל בדיוק את אותו סכום.
          </p>
          ...
          <p className="verdict">
            {club.adv > 0
              ? <>עם אותו כסף, הסגל של המנוע היה מייצר <b className="blue">{pct(club.adv)} יותר</b>.</>
              : <>הפעם הקבוצה האמיתית עשתה עבודה טובה יותר.</>}
          </p>
          <p className="fine">
            המספרים הם PIR למשחק, מדד התרומה הרשמי של היורוליג, לפי מה
            שהשחקנים באמת עשו באותה עונה. קבוצה אחת היא מדגם קטן, אז כדאי
            להסתכל גם על התמונה הכללית.
          </p>
      ...
      <section className="card">
        <h2>כמה דברים שכדאי לדעת</h2>
        <ul className="trust">
          <li><span className="ic" aria-hidden>⚖️</span><div>
            <b>השוואה הוגנת.</b> המנוע בחר את השחקנים לפני העונה, בלי לדעת
            מי ייפצע ומי יפתיע. כשהוא תכנן דקות לשחקן שבסוף לא שיחק, הוא
            שילם על זה.
          </div></li>
          <li><span className="ic" aria-hidden>🪑</span><div>
            <b>ספסל קצר.</b> למנוע יש 12 שחקנים, ולקבוצה אמיתית 15 עד 20.
            בגלל זה כ-14% מהדקות שהוא תכנן הלכו לשחקנים שנפצעו, ועדיין הוא
            יצא טוב יותר.
          </div></li>
          <li><span className="ic" aria-hidden>🎚️</span><div>
            <b>תלוי בהנחה אחת.</b> הגבלנו כמה דקות מותר לתת לכוכבים, לפי מה
            שקבוצות עשו בפועל. אם מגבילים עוד קצת, היתרון קטן בערך בשליש.
          </div></li>
          <li><span className="ic" aria-hidden>🔋</span><div>
            <b>בלי עייפות.</b> ניסינו למדוד עייפות מנתוני המשחקים, ולא הצלחנו
            להפריד אותה ממאמן שפשוט משאיר על המגרש את מי שחם. זה לא אומר
            שאין עייפות.
          </div></li>
        </ul>
      </section>
      ...
      {struct && (
        <section className="card">
          <h2>זה לא רק הכללים</h2>
          <p className="sub">
            אולי כל סגל שעומד באותם כללים היה מנצח? בדקנו: בחרנו סגל אקראי,
            עם אותו תקציב ואותם כללים. הוא הפסיד לקבוצה האמיתית, בערך{" "}
            <b dir="ltr">{f(Math.abs(struct.wins), 1)}</b> ניצחונות פחות בעונה.
            כלומר ההבדל הוא בבחירת השחקנים.
          </p>
          <p className="fine">
            הבדיקה הזאת נעשתה בשיטת הניקוד הקודמת. הכיוון ברור, ובגרסה הבאה
            נריץ אותה שוב.
          </p>
        </section>
      )}
      ...
      <section className="card">
        ...
        <p className="sub">
          בסוף העונה רואים שבערך שליש מהתקציב הלך לשחקנים שכמעט לא תרמו.
          אבל זה לא באמת בזבוז: מראש אי אפשר לדעת מי אלה יהיו, וספסל הוא
          גם ביטוח לפציעות. זה בדיוק המחיר שהמנוע שילם כשהחזיק רק 12 שחקנים.
        </p>
      </section>
      ...
      <details className="card deep">
        ...
        <div className="grid3">
          <Fact v={d.benchmark.n} l="קבוצות ועונות שנבדקו"
            h="כל קבוצה, בתקציב האמיתי שלה, בשתי עונות" />
          <Fact v={`+${pct(d.benchmark.adv_cap_pct)}`} l="יתרון בתפוקה, חציון"
            h="אף צד לא ידע מראש מי יצליח ומי ייפצע" />
          <Fact v={`${d.benchmark.n_win}/${d.benchmark.n}`} l="קבוצות שבהן המנוע עדיף"
            h="בכל מקרה נמצא הפתרון הטוב ביותר שקיים, ולא קירוב" />
        </div>
        <p className="sub">
למנוע יש שני כללים שהוא לא יכול לעקוף. הראשון: על המגרש יש כדור
          אחד, אז חמשת השחקנים לא יכולים לסיים יחד יותר התקפות ממה שאפשר.
          השני: ריכוז הדקות בשחקנים המובילים לא עולה על מה שקבוצות אמיתיות
          עשו בפועל.
        </p>
        ...
        {d.stability && (
          <>
            ...
            <p className="sub">
              חלק מהקשר בין תפוקה לניצחונות נובע פשוט מכך שקבוצות חזקות גם
              עשירות. אחרי שמנכים את התקציב, נשאר מהקשר הזה:{" "}
              {Object.entries(d.stability.ratio_by_season)
                .map(([k, v]) => `${k}: ${pct(v)}`).join(" · ")}. ההבדל בין
              העונות אינו מובהק (<span dir="ltr">p = {d.stability.permutation_p}</span>),
              ולכן הקשר נאמד על כל העונות יחד.
            </p>
          </>
        )}
