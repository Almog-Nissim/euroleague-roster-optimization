...
def build_pool(test, train_max, feat, anch, pos, ps, with_newcomers=True,
    ...
    # ------------------------------------------------------------------
    # ------------------------------------------------------------------
    # 🔴 יום 15 — הנרמול הופך לברירת מחדל תחת המפרט החדש
    #
    # `cost_market` מתמחר ב-α הממוצע, וזו **רמת מחירים ביורו**.
    # מבחן הקבלה (refit_acceptance) בדק אם מותר לרשום יורו על הציר
    # ונכשל בשלושת הסף: קריאת הציר כיורו נותנת MAE 8.57M€ מול
    # התקציבים בפועל, כי המודל מתמחר כל מועדון במחיר ליגה ממוצע
    # ולכן מכווץ את הפיזור **בין** מועדונים — בדיוק מה ש-ADR 0001
    # לא התיימר לתקן.
    #
    # ⇒ הרמה נזרקת, הצורה נשמרת. המחלק הוא ממוצע המאגר של אותה
    #   עונה, כך שיחידת הציר היא "שחקן מאגר ממוצע" — פיזית,
    #   בת-השוואה בין עונות, ולא תלוית מועדון. זה בדיוק הנימוק
    #   שנרשם כאן ביום 11; שם הוא הושאר כבוי כדי לא להזיז מספרים
    #   קיימים, וכאן המספרים זזים ממילא.
    #
    # המפרט הישן נשאר בלי נרמול, אחרת שחזור מספרי יום 14 נשבר.
    # ------------------------------------------------------------------
    if normalise_cost is None:
        normalise_cost = (ob.COST_SPEC == "market")
    info["normalised"] = bool(normalise_cost)
