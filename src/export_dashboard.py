...
⛔ הרוויה ביורו. שלושה מפרטי כיול נותנים 19.3 / 24.5 / 30.5.
   ציר התקציב ביחידות מנורמלות בלבד. תוקף: טווח התקציבים שמועדונים
   אמיתיים הוציאו (meta.observed_budget), לא 19.5 — שהיה גבול כיול היורו.
...
def main() -> int:
    ...
    # v1.0 (שלב 2, Q2): "חופשי − אקראי" (+6.61) הוסר מהתצוגה ומ-FROZEN. הוא
    # המנוע החופשי, שהכותרת שלו הוסרה, ולצד +5.01 הוא נקרא כאילו החלק גדול
    # מהשלם. מודל אפס תחת ADR 0006 ברשימת v2.
        ...
        "decomposition": [
            {"step": "מבנה האילוצים לבדו (סגל אקראי)",
             ...
             "note": "אקראי גרוע מהמועדון — האילוצים אינם מייצרים "
                     "את היתרון. נמדד בקונבנציה הקודמת; הסימן הוא "
                     "מה שחשוב, וגזירה מחדש ברשימת v2"},
        ],
    }
    ...
    if uc is not None and hx is not None:
        ...
        # 🔴 v1.0 (שלב 2, Q1): הלשונית "מנוע מול מציאות" לפי ADR 0006 —
        #    המועדון על מה ששיחק, המנוע של הכותרת על התוכנית שלו. קודם:
        #    usage_constrained_results + why_100, הקונבנציה הישנה, כך
        #    שהמספר של כל מועדון לא התיישב עם הכותרת שמעליו.
        D["clubs"] = [
            dict(season=int(r.season), club=r.club,
                 budget=round(float(r.budget), 2),
                 q_club=round(float(r.q_club_actual), 2),
                 q_engine=round(float(r.q_plan), 2),
                 adv=round(float(r.adv_F_exact), 4))
            for r in hx.itertuples()]
        print(f"  {len(D['clubs'])} עונות-מועדון יוצאו")

    # ------------------------------------------------ עקומת התקציב
    # 🔴 v1.0 (שלב 2, Q6). כאן יוצאה budget_curve.csv — עקומה מלפני
    #    הריפיט, שאף רכיב בממשק לא ציר — יחד עם "תוקף עד 19.5" מוקלד. 19.5
    #    היה גבול כיול היורו, לא גבול התוקף של הציר המנורמל, ו-4 מתוך 38
    #    מועדונים אמיתיים הוציאו יותר ממנו. עכשיו: הטווח שמועדונים אמיתיים
    #    הוציאו בפועל, מהנתונים. מחוץ לו — אקסטרפולציה.
    h("4. טווח התקציבים שנצפה")
    if hx is not None:
        D["meta"]["observed_budget"] = [round(float(hx.budget.min()), 2),
                                        round(float(hx.budget.max()), 2)]
        print(f"  {D['meta']['observed_budget'][0]}–"
              f"{D['meta']['observed_budget'][1]} יחידות מנורמלות · "
              f"{len(hx)} עונות-מועדון")
    ...
    #    אינה מגבלה מוצהרת.
    # 🔴 v1.0 (שלב 2, Q5): שלוש האזהרות שתוכנית הסגירה דורשת — ראשונות,
    #    כי "זר מבין תוך 30 שניות". עומק ועייפות היו חסרים לגמרי.
    # v1.0, שלב א': ניסוח פשוט, בלי מקפים. "x" (הפירוט הטכני) לא השתנה.
    D["limitations"] = [
        {"t": "למנוע יש 12 שחקנים, לקבוצה אמיתית 15 עד 20",
         "p": "המנוע מחלק את הדקות בין אותם 12 שחקנים ולא קונה עוד. בדקנו "
              "כמה זה עולה לו: כ-14% מהדקות שהוא תכנן הלכו לשחקנים שבסוף "
              "לא היו זמינים, והדקות האלה נזקפו לרמת שחקן חלופי: "
              "התפוקה האופיינית של שחקן שמחתימים באמצע העונה. גם "
              "אחרי זה הוא יצא טוב יותר.",
         "x": "ADR 0006. ניקוד עומק כביטוח (מונטה קרלו על זמינות) "
              "ברשימת v2."},

        {"t": "אין במודל עייפות",
         "p": "תכננו להוריד את התפוקה של שחקן מעל 30 דקות, ומדדנו את זה. "
              "לפי כלל שקבענו מראש, התוצאה לא הצדיקה להכניס את זה. מנתוני "
              "משחק שלם אי אפשר להבדיל בין שחקן עייף לבין מאמן שמשאיר על "
              "המגרש את מי שחם. זה אינו אומר שעייפות אינה קיימת.",
         "x": "ADR 0004. בחינה מחדש רק עם נתוני play-by-play, בתוך "
              "המשחק."},

        {"t": "התוצאה תלויה בכמה דקות מותר לתת לכוכבים",
         "p": "הגבלנו את הדקות של השחקנים המובילים לפי מה שהקבוצות הכי "
              "קיצוניות בליגה עשו בפועל. כשמגבילים עוד 5%, היתרון יורד "
              "מ-13.9% ל-9.8% (במדגם של 12 קבוצות). כשמשחררים 5%, הוא לא "
              "עולה. השארנו את הגבול לפי מה שקרה בפועל, ולא בחרנו אותו "
              "לפי התוצאה.",
         "x": "slack_sensitivity.py, 24/24 הוכחו אופטימליים. ADR 0005, "
              "run matrix b."},

        {"t": "המנוע מניח שכל שחקן פנוי",
         "p": "לשחקן אמיתי יש חוזה, משפחה והעדפות. המנוע בוחר מכל מי "
              "ששיחק בליגה, כאילו כולם על המדף. זו לא טעות בקוד. פשוט אין "
              "נתונים על מי באמת היה פנוי.",
         "x": "הנחת מודל מוצהרת, נבחרה ביום 3 יחד עם דחיית מסלול C."},

        {"t": "אי-הוודאות גדולה יותר ממה שנראה",
         "p": "כשמריצים את אותו ניתוח בשלוש דרכים סבירות, התשובות רחוקות "
              "זו מזו יותר מטווח השגיאה שמופיע לידן. כלומר הבחירה איך "
              "למדוד משפיעה יותר מגודל המדגם.",
         "x": "רצועת הרוויה 6.8M€ מול פער 11M€ בין שלושה מפרטי כיול "
              "(19.3 / 24.5 / 30.5). רגרסיה אינה מודדת את זה."},

        {"t": "ההמרה לניצחונות נמתחת מעבר למה שראינו",
         "p": "הקשר בין תפוקה לניצחונות נלמד מהבדלים קטנים בין קבוצות, "
              "והמנוע מייצר הבדלים גדולים בהרבה. קצת כמו ללמוד על גובה "
              "מילדים ולנבא ממנו גובה של מבוגרים.",
         "x": "נאמד על פיזור ניקוד 6–10 נקודות, מוחל על פער 18.6. "
              "אזור האקסטרפולציה מסומן באפור בגרפים."},

        {"t": "קבוצה טובה היא לא רק סגל טוב",
         "p": "לקבוצה עם תפוקה גבוהה יש בדרך כלל גם יותר כסף, מאמן טוב "
              "יותר ומתקנים טובים יותר. ניכינו חלק מזה, לא את הכול. לכן "
              "המספר הוא תקרה ולא הערכה מדויקת.",
         "x": "השיפוע נאמד בין מועדונים. בקרת log(תקציב) מנכה חלק "
              "מההשפעה; מאמן, לכידות ובריאות נשארים בפנים."},

        {"t": "שחקן בעונה הראשונה שלו: המחיר הוא ניחוש",
         "p": "מודל המחיר לומד מהעונה הקודמת של השחקן. למי שמגיע ליורוליג "
              "בפעם הראשונה אין עונה קודמת, אז הוא מקבל הערכה גסה.",
         "x": "המודל נשען על pir_lag. 107 מתוך 335 שחקני המאגר אינם "
              "ב-player_features כלל."},

        {"t": "כל הליגה מתומחרת באותו שוק",
         "p": "בפועל קבוצה בספרד משלמת אחרת מקבוצה בישראל, בגלל מיסים, "
              "שחקנים מקומיים ושחקנים שגדלו במועדון. המנוע מתעלם מזה "
              "בכוונה, כי השאלה היא מה אפשר לקנות בכסף ולא כמה זה עולה "
              "לקבוצה מסוימת. עקומת המחיר נלמדת מ-13 קבוצות, אבל רמת "
              "המחירים שבה משתמש המנוע היא אחת לכל הליגה.",
        ...
        {"t": "אין כאן סכומים ביורו, בכוונה",
         "p": "התקציב מוצג כאחוז מהתקציב של קבוצה ממוצעת. ניסינו להמיר "
              "ליורו, ושלוש דרכים סבירות נתנו תשובות שרחוקות 11 מיליון זו "
              "מזו. לכן הורדנו את היורו לגמרי במקום להציג אותו עם טווח "
              "ענק.",
