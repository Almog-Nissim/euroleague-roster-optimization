...
def score_actual(df, ppm_col, minutes_col, repl=None):
    ...
    165.5 דקות בממוצע מול 125.7 שהיא שיחקה — מכנה מנופח ב-31.6%,
    ו-37 מתוך 38 המועדונים (כולם חוץ מ-2024 BER) מפרים את האילוץ
    שנבנה מהמקסימום שלהם עצמם. (תוקן אחרי code review: 177.0 /
    40.8% / 38 מתוך 38 נמדדו על תחליף חמדני בלי תקרות עמדה.)
    """
    ppm = df[ppm_col].values
